...
def get_notes():
    ...
    for file in glob.glob("midi/*.mid"):
        ...
        highestPitches = [] #to draw melody part
        highestMean = 0
        melodyPart = None
            ...
            for item in part.flat:
                if not isinstance(item, m21.note.Rest):
                    if isinstance(item, m21.note.Note):
                        partPitches.append(item.pitch.ps)
                    elif isinstance(item, m21.chord.Chord):
                        chordList = []
                        for nt in item:
                            chordList.append(nt.pitch.ps)
                        partPitches.append(np.mean(chordList))
            avgPitch = np.mean(partPitches)
        ...
        for element in melodyPart.flat:
